...
class Security extends Component {
  ...
  onInitHistoryList(type) {
    getHistoryList(type).then((result) => {
      if (result.data) {
        ...
        this.setState({
          history: historyList,
          mainHis: historyList.length > 0 ? historyList[0] : {},
        });
      }
    });
  }
  ...
  onHandleInputCard = () => {
    ...
    inputCard({ info: cardId, type: type })
      .then((result) => {
    ...
    return (
      <React.Fragment>
        <div
          className={
            !mainScreen
              ? 'p-t-10 header-security'
              : 'p-t-10 header-security security-search'
          }
        >
          <audio
          ...
          <div className="type-div">
            <RadioButton
              check={this.state.type === 'in-dormitory'}
              value={'in-dormitory'}
              isRadioChk={(e) => this.radioCheck(e)}
              className="type-radio-button"
              name="type"
              label="Vào ký túc xá"
            />
            <RadioButton
              check={this.state.type === 'out-dormitory'}
              value={'out-dormitory'}
              isRadioChk={(e) => this.radioCheck(e)}
              className="type-radio-button"
              name="type"
          ...
          <div>
            <Link
              to={mainScreen ? '/security' : '/security/student'}
              onClick={() => {
                if (mainScreen)
                  document.addEventListener('keydown', this.onKeyDown, false);
                else
        ...
        {!this.state.mainScreen ? (
          <div className={'content-body-security'}>
            <Row>
                ...
                <Row>
                  <Col md={6}>
                    <input
                      ...
                      onChange={(e) => this.getInput(e.target)}
                      autoFocus={true}
                      onKeyPress={(e) => this.onEnterInput(e)}
                    />
                  </Col>
                  <Col md={5}>
                    <div style={{ cusor: 'pointer' }} onClick={this.hideLichSu}>
              ...
              <Col md={this.state.isHide ? 12 : 9}>
                <Row className="card-large">
                  <Col md={6} className={'col-outer'}>
                    <div className={'img-css'}>
                      {!this.state.notFound ? (
                        mainHis.profile && mainHis.profile.img ? (
                          <img alt={mainHis.MSSV} src={mainHis.profile.img} />
                        ) : (
                  ...
                  >
                        ...
                        <div className="card-content">
                          ...
                          <div className="card-body-content">
                            <Col md={12} className={'info'}>
                              Trường:{' '}
                              <span>
                                {mainHis.profile.truong
                                  ? mainHis.profile.truong.tenTruong
                                  : 'Chưa cập nhật'}
                              </span>
                            </Col>
                            <Col md={12} className={'info'}>
                    ...
                    <div className="decor-end-line"></div>
                  </Col>
                </Row>
              </Col>
              {!this.state.isHide && (
                        ...
                        return (
                          <div className={'item-history'} key={index}>
                            <div className={'item-image'}>
                              {value.profile && value.profile.img ? (
                                <img
                                  alt={value.profile.hoTen}
                                  src={value.profile.img}
                                />
                              ) : (
                                <div>Chưa cập nhật ảnh</div>
                              )}
                            </div>
                            <div className={'item-text'}>
                              <div>{value.profile.hoTen}</div>
                              <div>
                                {value.profile.idPhong
                                  ? 'Phòng: ' + value.profile.idPhong.tenPhong
                                  : 'Phòng: Chưa cập nhật'}
                              </div>
                              <div>
          ...
          <Layout>
            <Route
              ...
              component={Authorization(roles)(infoStudent, 'SV01')}
            />
            <Route
              exact
              path={`${this.props.match.url}/student/detail/:mssv`}
              component={Authorization(roles)(InfoStudentDetail, 'SV02')}
            />
            <Route
              exact
              path={`${this.props.match.url}/history`}
              component={Authorization(roles)(History, 'LS01')}
            />
          </Layout>
        )}
      </React.Fragment>
